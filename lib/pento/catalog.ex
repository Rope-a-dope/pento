defmodule Pento.Catalog do
  ...
  @doc """
  Decreases the price of the given product by the specified amount.

  ## Parameters
  - product: The product to be updated.
  - amount: The amount by which the product's price should decrease.

  ## Returns
  - {:ok, product} if the price was successfully updated.
  - {:error, changeset} if the price decrease is invalid.
  """
  def markdown_product(product, amount) when is_number(amount) and amount > 0 do
    current_price = product.unit_price

    new_price = current_price - amount

    changeset = Product.price_decrease_changeset(product, %{unit_price: new_price})
  ...
  def markdown_product(_, _), do: {:error, "Invalid amount"}
